...
Я использовал sqlite3 в качестве базы данных.Для создания базы данных я использовал миграции , они лежат в папке migrations.
В бд есть две таблицы , первая для пользователей и вторая для выражений.Связаны они по ключу user_id.После регистрации пользователя , я кладу в бд его email и пароль в виде хеша.После авторизации я ищу пользователя в бд по email и сверяю его пароль с хешем из бд.
В бд я также кладу выражение , его результат и его статус.Так как при каждом запросе отправляется JWT токен , я проверяю валиден ли этот токен , беру из этого токена email пользователя и либо вычисляю для него выражения и кладу их в бд , либо получаю выражения из бд.
...
## Frontend
Он работал без авторизации , но сейчас с тем , что нужно отправлять токен в каждом запросе , он не работает, возможно я его исправлю в будущем  
Можете просто посмотреть какой там дизайн
